...
def train_item_based_recommendation_model(
    ...
    _user_profiles["user_objectid"] = user_encoder.transform(_user_profiles["user_objectid"])
    _all_item_profiles["item_objectid"] = item_encoder.transform(_all_item_profiles["item_objectid"])
    _user_profiles = _user_profiles.sort_values(by="user_objectid").reset_index(drop=True)
    _all_item_profiles = _all_item_profiles.sort_values(by="item_objectid").reset_index(drop=True)
    
    # Ensure both profiles DataFrames have the same naming convention for columns
    new_user_columns = [col.replace("user_", "") if col != "user_objectid" else "user_objectid" for col in _user_profiles.columns]
    new_item_columns = [col.replace("item_", "") if col != "item_objectid" else "item_objectid" for col in _all_item_profiles.columns]
