...
class RecommendationDataset(Dataset):
    def __init__(
        self, user_features: np.ndarray, item_features: np.ndarray, labels: np.ndarray
    ) -> None:
    ...
    def __getitem__(self, idx) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        return (
            torch.tensor(user_feature, dtype=torch.float),
            torch.tensor(item_feature, dtype=torch.float),
...
def train_MLP_model(
    ...
    train_dataset = RecommendationDataset(
        train_user_features_df.values, train_item_features_df.values, y_train
    )
...
def load_MLP_model(
    filename: str, data: pd.DataFrame, load_full_model: bool = True
) -> MLP:
