...
def build_user_profile(user_features_df: pd.DataFrame) -> pd.DataFrame:
    ...
    def build_row_user_profile(row: pd.Series) -> dict[str, str]:
        ...
        # Demographies
        row_demography_array = ast.literal_eval(row["demography_array"])
        row_score_array_demography = ast.literal_eval(row["score_array_demography"])
        for demography, score in zip(row_demography_array, row_score_array_demography):
            profile[f"user_demography_['{demography}']"] = score
        
        # Brands
        row_brand_array = ast.literal_eval(row["brand_array"])
        row_score_array_brand = ast.literal_eval(row["score_array_brand"])
    ...
    user_profiles.insert(0, "user_objectid", user_features_df["user_objectid"])
...
def build_item_profile(item_data_df: pd.DataFrame) -> pd.DataFrame:
    ...
    def build_row_item_profile(row: pd.Series) -> dict[str, str]:
        ...
        row_metadata = {} if pd.isna(row["metadata"]) else ast.literal_eval(row["metadata"])
    ...
    item_profiles.insert(0, "item_objectid", item_data_df["item_objectid"])
...
def get_all_demographies_and_brands(
    ...
    for _, row in user_features_df.iterrows():
        demography_array = ast.literal_eval(row["demography_array"])
        demographies.update(demography_array)
        brand_array = ast.literal_eval(row["brand_array"])
...
def parse_demographic_or_brand(input_string: str):
    ...
    if len(parts) != 2:
        raise AssertionError(f"Input string: '{input_string}', does not follow the expected format.")
...
def get_preprocessed_data(
    ...
    all_item_features = sorted(list(set(item_profiles.columns) | set(live_item_profiles.columns)))
    item_profiles_extended = item_profiles.reindex(columns=all_item_features, fill_value=float(0))
    live_item_profiles_extended = live_item_profiles.reindex(columns=all_item_features, fill_value=float(0))
    all_item_profiles = pd.concat([item_profiles_extended, live_item_profiles_extended], axis=0).reset_index(drop=True)
    ...
    live_item_profiles = live_item_profiles.reindex(columns=all_item_features, fill_value=0)
    all_item_profiles = all_item_profiles.reindex(columns=all_item_features, fill_value=0)
...
def get_dataset(
    ...
    negative_df = generate_negative_samples(user_item_interactions_df, num_negatives_per_positive=1)
    ...
    all_features = sorted(list(set(user_profiles.columns) | set(item_profiles.columns) | set(live_item_profiles.columns) | {"label"}))
    ...
    all_items = sorted(list(set(item_data_for_interactions_df["item_objectid"].values) | set(live_item_data_df["item_objectid"].values)))
...
def generate_negative_samples(interactions_df: pd.DataFrame, num_negatives_per_positive: int = 1) -> pd.DataFrame:
    ...
    for user in unique_users:
        user_positive_items = sorted(list(interactions_df[interactions_df["user_objectid"] == user]["item_objectid"].unique()))
        possible_negatives_items = sorted(list(set(unique_items) - set(user_positive_items)))
        
        if num_negatives_per_positive <= len(possible_negatives_items):
            negative_items = random.sample(possible_negatives_items, num_negatives_per_positive * len(user_positive_items))
            for item in negative_items:
                negative_samples.append({"user_objectid": user, "item_objectid": item, "label": 0.0})
...
def get_positive_samples(interactions_df: pd.DataFrame) -> pd.DataFrame:
    ...
    positive_samples["label"] = positive_samples["interaction_type"].map({"order": 1.0, "wishlist": 0.7})
    
    # Select only the required columns
    positive_samples = positive_samples[["user_objectid", "item_objectid", "label"]]
    assert positive_samples.notna().all().all(), "There are NaN values in the DataFrame"
    
    return positive_samples


def get_subset_by_column_substring(df: pd.DataFrame, column_name_substring: str) -> pd.DataFrame:
    """
