...
from utils.preprocess import get_subset_by_column_substring, parse_demographic_or_brand
...
def evaluate_recommendation(
    ...
    previous_interactions: pd.DataFrame | None = None,
) -> None:
    ...
    """
    item_profile = all_item_profiles[
        all_item_profiles["item_objectid"] == item_objectid
    ]
    assert item_profile.shape[0] == 1, (
        f"Item {item_objectid} did not return a single row from all_item_profiles."
    )
    ...
    assert len(item_features) == 2, (
        f"Item {item_objectid} shoud have exactly one brand and one demography, not {item_features}."
    )
    item_feature_names = list(item_features.keys())
    is_demography_first = "demography" in item_feature_names[0]
    item_demography = (
        item_feature_names[0] if is_demography_first else item_feature_names[1]
    )
    ...
    assert user_profile.shape[0] == 1, (
        f"User {user_objectid} did not return a single row from user_profiles."
    )
    ...
    user_features[f"user_demography_['{item_demography}']"] = float(
        all_user_features[f"user_demography_['{item_demography}']"]
    )
    user_features[f"user_brand_['{item_brand}']"] = float(
        all_user_features[f"user_brand_['{item_brand}']"]
    )
    print(
        f"User {user_objectid} has the following relevant features for item {item_objectid}: {user_features}"
    )
    ...
    if previous_interactions is not None:
        ...
        for _, row in previous_interactions.iterrows():
            prev_item_df = all_item_profiles[
                all_item_profiles["item_objectid"] == row["item_objectid"]
            ]
            assert prev_item_df.shape[0] == 1, (
                f"Item {row['item_objectid']} did not return a single row from all_item_profiles."
            )
            prev_item_profile_no_id_column = prev_item_df.drop(
                columns=["item_objectid"]
            )
            ...
            assert len(prev_item_features) == 2, (
                f"Item {row['item_objectid']} shoud have exactly one brand and one demography, not {prev_item_features}."
            )
            prev_item_feature_names = list(prev_item_features.keys())
            is_demography_first = "demography" in prev_item_feature_names[0]
            prev_item_demography = (
                prev_item_feature_names[0]
                if is_demography_first
                else prev_item_feature_names[1]
            )
            prev_item_brand = (
                prev_item_feature_names[1]
                if is_demography_first
                else prev_item_feature_names[0]
            )
        ...
        print(
            f"User {user_objectid} has {num_previous_interactions_with_item_demography} previous interactions with: {item_demography}."
        )
        print(
            f"User {user_objectid} has {num_previous_interactions_with_item_brand} previous interactions with: {item_brand}."
        )
    return
...
def evaluate_user_recommendations(
    ...
    ml_model: np.ndarray | LinearRegression | MLP,
    user_encoder: LabelEncoder,
    item_encoder: LabelEncoder,
    user_profiles: pd.DataFrame,
    all_item_profiles: pd.DataFrame,
    previous_interactions: pd.DataFrame | None = None,
    ground_truth_for_unseen_items: pd.DataFrame | None = None,
    top_n: int = 10,
) -> None:
    ...
    Args:
        ...
        ml_model (np.ndarray | LinearRegression | MLP): Trained machine learning model.
    ...
    assert not user_columns, (
        f"Unseen items DataFrame should not contain any user features, only item features. User features in unseen_items: {user_columns}"
    )
    ...
    user_profile.iloc[0, user_profile.columns.get_loc("user_objectid")] = (
        user_encoded_label
    )

    user_profile_np = user_profile.iloc[0].to_numpy()
    repeated_user_profile = np.repeat(
        user_profile_np.reshape(1, -1), unseen_items.shape[0], axis=0
    )
    user_profile = pd.DataFrame(
        repeated_user_profile, columns=user_profile.columns, dtype=float
    )
    ...
    assert unseen_data["user_objectid"].dtype.type is np.int64, (
        f"'user_objectid' should be of type 'np.int64', not {unseen_data['user_objectid'].dtype.type}."
    )
    assert unseen_data["item_objectid"].dtype.type is np.int64, (
        f"'item_objectid' should be of type 'np.int64', not {unseen_data['item_objectid'].dtype.type}."
    )
    unseen_items_objectids = item_encoder.inverse_transform(
        unseen_items["item_objectid"].values
    ).tolist()

    # Predict recommendation scores for unseen items
    assert "label" not in unseen_data.columns, (
        "Unseen data DataFrame should not contain a 'label' column."
    )
    if isinstance(ml_model, LinearRegression):
        pred = ml_model.predict(unseen_data).tolist()
    elif isinstance(ml_model, np.ndarray):
        item_encoded_label = unseen_data["item_objectid"].values
        pred = ml_model[user_encoded_label, item_encoded_label].tolist()
    elif isinstance(ml_model, MLP):
        ...
        unseen_user_features = torch.tensor(
            unseen_user_features_df.values, dtype=torch.float
        )
        unseen_item_features = torch.tensor(
            unseen_item_features_df.values, dtype=torch.float
        )
        with torch.no_grad():
            pred = ml_model(unseen_user_features, unseen_item_features).numpy().tolist()
    else:
        raise AssertionError(
            f"'ml_model' must be of some type: {[np.ndarray, LinearRegression, MLP]}. Not {type(ml_model)}"
        )
    ...
    if ground_truth_for_unseen_items is not None:
        result = list(
            zip(
                unseen_items_objectids,
                ground_truth_for_unseen_items.values.tolist(),
                pred,
            )
        )
    else:
    ...
    for n in range(min(top_n, len(ranked_result))):
        ...
        if ground_truth_for_unseen_items is not None:
            nth_recommendation_dict = dict(
                zip(["item_objectid", "label", "score"], nth_recommendation)
            )
        else:
            nth_recommendation_dict = dict(
                zip(["item_objectid", "score"], nth_recommendation)
            )
        print(
            f"Recommendation {n + 1} for user {user_objectid}: {nth_recommendation_dict}"
        )
