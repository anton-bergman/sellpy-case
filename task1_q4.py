...
def analyze_demographic_interest(
    user_features_csv_path: str, target_demographic: str
) -> tuple[int, int]:
    """
    ...
    for _, row in user_features_df.iterrows():
        demography_array_str = row.loc["demography_array"]
...
def main() -> None:
    ...
    num_users_with_interest, num_users_with_primary_interest = (
        analyze_demographic_interest(user_features_path, "Kids")
    )

    print(f"Number of users with 'Kids' interest: {num_users_with_interest}")
    print(
        f"Number of users with 'Kids' as main interest: {num_users_with_primary_interest}"
    )
