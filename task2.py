...
import torch
from utils.preprocess import read_csv_data, get_preprocessed_data, get_dataset, get_subset_by_column_substring
from utils.evaluate import evaluate_user_recommendations
from models.linear_regression_model import train_linear_regression_model, load_linear_regression_model
from models.user_item_similarity_model import train_item_based_recommendation_model
from models.mlp import train_MLP_model, load_MLP_model
...
def main() -> None:
    ...
    user_profiles, item_profiles, live_item_profiles, all_item_profiles = get_preprocessed_data(
        user_features_df,
    ...
    split_data: tuple[pd.DataFrame, pd.DataFrame] = train_test_split(data_df, test_size=0.2, random_state=SEED)
    train_df, test_df = split_data[0].reset_index(drop=True), split_data[1].reset_index(drop=True)
    
    # Model Training
    if MODEL == ModelType.MLP:
        if MODE == Mode.TRAIN_ON_FULL_DATA:
            model = train_MLP_model(train_df=data_df, save_model_to="./saved_models/MLP_model_full_data.pth")
        elif MODE == Mode.TRAIN_ON_SPLIT_DATA:
            model = train_MLP_model(train_df, test_df, save_model_to="./saved_models/MLP_model_split_data.pth")
        elif MODE == Mode.LOAD_MODEL_FOR_FULL_DATA:
            model = load_MLP_model("./saved_models/MLP_model_full_data.pth", data_df, load_full_model=True)
        else:
            model = load_MLP_model("./saved_models/MLP_model_split_data.pth", test_df, load_full_model=False)
    elif MODEL == ModelType.LINEAR_REGRESSION:
        if MODE == Mode.TRAIN_ON_FULL_DATA:
            model = train_linear_regression_model(data_df, save_model_to="./saved_models/linear_regression_model_full_data.pkl")
        elif MODE == Mode.TRAIN_ON_SPLIT_DATA:
            model = train_linear_regression_model(train_df, test_df, save_model_to="./saved_models/linear_regression_model_split_data.pkl")
        elif MODE == Mode.LOAD_MODEL_FOR_FULL_DATA:
            model = load_linear_regression_model("./saved_models/linear_regression_model_full_data.pkl")
        else:
            model = load_linear_regression_model("./saved_models/linear_regression_model_split_data.pkl", test_df)
    else:
        model = train_item_based_recommendation_model(
            ...
            user_encoder,
            item_encoder,
            calc_metrics=(MODE == Mode.TRAIN_ON_SPLIT_DATA or MODE == Mode.LOAD_MODEL_FOR_SPLIT_DATA)
        )
    

    user_encoded_label = user_encoder.transform([USER_OBJECTID])[0]
    positive_user_interactions = test_df[(test_df["user_objectid"] == user_encoded_label) & (test_df["label"] != 0)]
    assert user_encoded_label in positive_user_interactions["user_objectid"].values, f"'user_objectid' {positive_user_interactions} not found in positive interactions of the test set."
    
    X_test_unseen_items = get_subset_by_column_substring(positive_user_interactions, "item_")
    y_test_unseen_items = positive_user_interactions["label"]
    live_unseen_items = live_item_profiles.copy()
    live_unseen_items["item_objectid"] = item_encoder.transform(live_unseen_items["item_objectid"])
    
    previous_interactions = train_df[train_df["user_objectid"] == user_encoded_label]
    item_objectids = item_encoder.inverse_transform(previous_interactions["item_objectid"].values)
