...
def get_ordered_and_wishlisted_items_per_country(user_item_interactions_csv_path: str, user_features_csv_path: str) -> dict[str, dict[str, int]]:
    """
    ...
    merged_df = pd.merge(user_item_interactions_df, user_features_df, on="user_objectid", how="inner")

    # Group by country and interaction type
    grouped = merged_df.groupby(["country", "interaction_type"]).size().unstack(fill_value=0)
...
def main() -> None:
    ...
    country_interactions = get_ordered_and_wishlisted_items_per_country(user_item_interactions_path, user_features_path)
